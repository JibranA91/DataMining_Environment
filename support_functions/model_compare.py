...
def all_feature_models_validation(model_data, all_features_list, test_data_percentage=0.3,
    ...
    model_data = model_data[all_features_list]
    ...
    for i in range(loop_len):
        ...
        if is_decision_tree == '1':
            ...
            model_accuracy = fmv_decision_tree_output[0].loc['accuracy', ['precision']].to_string().replace('precision','').strip()
        ...
        if is_random_forest == '1':
            ...
            fmv_random_forest_output = random_forest(fmv_train_test_data[0],
                                                     ...
                                                     fmv_train_test_data[3],
                                                     fmv_train_test_data[4])
            model_accuracy = fmv_random_forest_output[0].loc['accuracy', ['precision']].to_string().replace('precision','').strip()
        ...
        if is_naive_bayes == '1':
            ...
            fmv_naive_bayes_output = naive_bayes(fmv_train_test_data[0],
                                                 ...
                                                 fmv_train_test_data[3],
                                                 fmv_train_test_data[4])
            model_accuracy = fmv_naive_bayes_output[0].loc['accuracy', ['precision']].to_string().replace('precision','').strip()
        ...
        if is_neural_network == '1':
            ...
            fmv_neural_network_output = neural_network(fmv_train_test_data[0],
                                                       ...
                                                       fmv_train_test_data[3],
                                                       fmv_train_test_data[4])
            model_accuracy = fmv_neural_network_output[0].loc['accuracy', ['precision']].to_string().replace('precision','').strip()
